...
def mock_get_requests(url, params):
    class MockRequestGetResponse: 
        ...
        def json(self):
            return self.data
    if url == "%s/members/me/boards" % config.TRELLO_BASE_URL: 
        return {
            # fake board response
        }
    elif url == '%s/boards/%s/lists' %(config.TRELLO_BASE_URL, config.TRELLO_BOARD_ID):
        response =  MockRequestGetResponse(fake_get_list_data, 200)
        return response